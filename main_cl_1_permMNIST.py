...
def run(args, verbose=False):
    ...
    param_stamp = get_param_stamp(
        ...
        batch_replay_n=args.batch_replay,
    )
    ...
    if args.visdom:
        ...
        # )
        graph_name = 'BIR_brn{}_permMNIST_1'.format(args.batch_replay)
        visdom = {'env': env_name + '_1', 'graph': graph_name}
...
if __name__ == '__main__':
    ...
    print('>> permMNIST batch_replay : {}'.format(args.batch_replay), BIR)
    
    torch.save(
        BIR, './store/permMNIST/bir_brn{}.pth'.format(args.batch_replay)
    )