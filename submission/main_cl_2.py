...
def handle_inputs():
    ...
    parser = options.add_replay_options(parser, **kwargs)
    parser = options.add_bir_options(parser, **kwargs)
    parser.add_argument('--internal-replay-off', action='store_true')
    parser.add_argument('--method', type=str, default='bir')
    parser.add_argument('--epsilon', type=float, default=0.1, dest="epsilon", help="-->  SI: dampening parameter")
...
def run(args, verbose=False):
    ...
    param_stamp = get_param_stamp(
        ...
        internal_replay_off=args.internal_replay_off
    )
    ...
    if args.visdom:
        ...
        # )
        graph_name = 'BIR_nI{}_CIFAR100_2'.format(args.internal_replay_off)
        visdom = {'env': env_name + '_2', 'graph': graph_name}
        print('visdom ', visdom)
    ...
    if args.train:
        if verbose:
            print("\nTraining...")
        # Train model
        initial_accs, initial_average_accs = train_cl(
            model, train_datasets, test_datasets=test_datasets, replay_mode=args.replay if hasattr(args, 'replay') else "none",
    ...
    #-------------------------------------------------------------------------------------------------#
    # TODO JINA add this to the plot
    #--------------------#
    #----- PLOTTING -----#
    #--------------------#
    ...
    if args.method == "bir":
        dic_path = './store/main_cl_2/train_nI{}.pth'.format(args.internal_replay_off)
    elif args.method == "bir_si":
        dic_path = './store/main_cl_2/train_{}_nI{}.pth'.format(args.method, args.internal_replay_off)
    else:
    ...
    print(">> saved {}".format(dic_path))
    
    return accs, average_accs, initial_accs, initial_average_accs
...
if __name__ == '__main__':
    ...
    ## BI-R
    if args.method =="bir":
        if not args.internal_replay_off:
            args.hidden=True
        ...
        store_path = './store/main_cl_2/accuracy_nI{}.pth'.format(args.internal_replay_off)
    elif args.method == "bir_si":
        args.si = True
        args.si_c = 100000000. # args.dg_c
        if not args.internal_replay_off:
            args.hidden=True
            args.dg_prop = 0.6 # args.dg_si_prop
        ## No internal replay
        else:
            args.hidden = False
            args.dg_prop = 0.8 # dg_prop_nI
        store_path = './store/main_cl_2/accuracy_{}_nI{}.pth'.format(args.method, args.internal_replay_off)
    else:
        raise Exception("method : {}".format(args.method))
    ...
    for seed in seed_list:
        args.seed = seed
        final_accs, final_average_accs, initial_accs, initial_average_accs = run(args, verbose=True)
        
        method_dict[seed] = {
            'final_accs' : final_accs,
            'final_average_accs' : final_average_accs,
            'initial_accs' : initial_accs,
            'initial_average_accs' : initial_average_accs
        }
        
    # torch.save(initial_accuracy, './store/main_cl_2/initial_accuracy_nI{}.pth'.format(args.internal_replay_off))
    
    torch.save(
        method_dict, store_path
    )
    
    print(">> saved at {}".format(store_path))
